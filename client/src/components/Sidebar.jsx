...
const Sidebar = () => {
  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-0 z-1'>
          ...
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.
            toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().
              includes(search.toLowerCase())).map((chat) => (
                <div key={chat._id} className='p-3 px-4 bg-white dark:bg-[#57317C]/10 
      ...
      {/* Community Images */}
      <div onClick={() => { navigate('/community') }} className='flex items-center gap-2 p-3 mt-4 
      ...
      <div onClick={() => { navigate('/credits') }} className='flex items-center gap-2 p-3 mt-4 
        border border-gray-300 dark:border-white/15 
