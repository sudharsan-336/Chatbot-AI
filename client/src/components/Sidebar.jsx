...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen ? 'max-md:-translate-x-full' : 'translate-x-0'}`}>
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.
            toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().
              ...
              key={chat._id} className='p-3 px-4 bg-white dark:bg-[#57317C]/10 
      ...
      border border-gray-300 dark:border-white/15 
      rounded-md cursor-pointer group hover:bg-gray-100 
      dark:hover:bg-white/10 transition-all'>
        <img src={assets.user_icon} className='w-7 rounded-full' alt="" />
        <p className='flex-1 text-sm dark:text-primary truncate'>{user ? user.name : 'Login your account'}</p>
        {user && <img src={assets.logout_icon} className='h-6 cursor-pointer hidden group-hover:block 
        invert dark:invert-0 p-1 rounded bg-black/10 
        dark:bg-white/20 transition-all' />}
      </div>
